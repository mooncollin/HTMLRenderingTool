	...
	Attributes.TabIndex, Attributes.Value, Attributes.Type
{
	...
	@SafeVarargs
    public Input(Map.Entry<String, ?>... entries)
    {
        this(null, entries);
    }
	
	public Input(Map<String, Object> attributes)
    {
        this(null, attributes);
    }
	
	@SafeVarargs
    public Input(String name, Map.Entry<String, ?>... entries)
    {
        this(name, Map.ofEntries(entries));
    }
	...
	public Input(String name, Map<String, Object> attributes)
	{
		...
		if(attributes != null && 
			!attributes.containsKey(Attributes.Name.ATTRIBUTE_NAME) &&
			name != null &&
			!name.isEmpty())
		{
			setName(name);
